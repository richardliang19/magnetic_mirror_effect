...
# 算出各初始動能
electronic,energy = difdegree(40)
for i in range(360):
    ws[f'C{i+2}'].value = 0.5*m*sqrt((electronic[i].v.x)**2+(electronic[i].v.y)**2)
...
for i in range(360): 
    ...
    ws[f'D{energyline}'].value = 0.5*m*sqrt((electronic[i].v.x)**2+(electronic[i].v.y)**2)
    ...
    if electronic[i].pos.x>50 and electronic[i].pos.x<150 and electronic[i].pos.y<75 and electronic[i].pos.y>-75:
        ...
        ws[f'B{correctline}'].value = f'{i} degree'
        wb.save("vpython_data.xlsx")
        incorrectline+=1
